...
const CACHE_KEY = "guidatv_data_v2";
const IMAGE_CACHE_KEY = "guidatv_images_tvmaze_v1";
const PX_PER_MIN = 2;
...
function categoryTone(category=""){
  ...
  if(/film|movie|cinema|serie|fiction|dramm|azione|thriller|crime/.test(c)) return "tone-c";
  if(/sport|calcio|basket|tennis|motori/.test(c)) return "tone-b";
  return "tone-a";
}
...
function saveImageCache(){
  const entries=Object.entries(state.imageCache).slice(-180);
...
async function fetchTvmazeImage(title){
  ...
  if(state.imageCache[key]) return state.imageCache[key].url||"";
  try{
    const r=await fetch(`https://api.tvmaze.com/search/shows?q=${encodeURIComponent(cleaned)}`);
    if(!r.ok) throw new Error("TVmaze");
    const rows=await r.json();
    let chosen=null;
    for(const row of rows.slice(0,5)){
      const name=norm(row?.show?.name||"");
      if(name===key||key.includes(name)||name.includes(key)){ chosen=row.show; break; }
    }
    const url=chosen?.image?.medium||chosen?.image?.original||"";
    state.imageCache[key]=url?{url,source:chosen.url||"https://www.tvmaze.com"}:{miss:true};
    saveImageCache();
    return url;
  }catch(e){ return ""; }
}
async function hydrateThumbnails(){
  const els=[...document.querySelectorAll(".thumb[data-title]")].filter(el=>!el.dataset.done).slice(0,10);
  for(const el of els){
    el.dataset.done="1";
...
function programmeHTML(channel,p,isNow=false){
  ...
  return `<article class="programme" data-programme="${esc(key)}">
    <div class="thumb" data-title="${esc(p.title||"")}" ${p.image?'data-done="1"':''}>${p.image?`<img src="${esc(p.image)}" alt="" loading="lazy">`:''}<span class="fallback">${fallback}</span><span class="time-tag">${fmtTime(p.start)}</span></div>
    <div>
...
function renderTimeline(){
  ...
  state.data.channels.forEach(ch=>{
    ...
    ps.forEach(p=>{
      ...
      const w=Math.max(40,((e-s)/60000)*PX_PER_MIN-3);
      blocks+=`<button class="tl-programme ${categoryTone(p.category)}" data-programme="${esc(programmeKey(ch.id,p))}" style="left:${left}px;width:${w}px"><strong>${esc(p.title||"Programma")}</strong><small>${fmtTime(p.start)} · ${esc(p.category||"")}</small></button>`;
    });
    const initials=ch.name.replace(/[^A-Za-z0-9]/g,"").slice(0,4).toUpperCase();
    rows+=`<div class="tl-row" style="width:${width}px"><div class="tl-channel">${ch.logo?`<img src="${esc(ch.logo)}" alt="${esc(ch.name)}">`:`<span>${esc(initials)}</span>`}</div>${blocks}</div>`;
  });
...
function openDetail(key){
  ...
  if(cached){ const img=document.createElement("img"); img.src=cached; img.alt=""; hero.prepend(img); }
  else fetchTvmazeImage(p.title||"").then(url=>{ if(url&&modal.classList.contains("open")){ const img=document.createElement("img"); img.src=url; img.alt=""; hero.prepend(img); } });
  modal.classList.add("open"); modal.setAttribute("aria-hidden","false");
}
function closeDetail(){ modal.classList.remove("open"); modal.setAttribute("aria-hidden","true"); }
function setView(view){
  state.view=view; state.query=""; $("#search").value=""; drawerOverlay.classList.remove("open"); render();
}
async function loadData(force=false){
  ...
  try{
    ...
    if(cached){ state.data=JSON.parse(cached); statusEl.textContent="Offline: ultimo palinsesto salvato"; render(); }
    else{ statusEl.textContent="Palinsesti non disponibili."; content.innerHTML=`<div class="empty">Aggiorna i palinsesti e riprova.</div>`; }
  }
}
